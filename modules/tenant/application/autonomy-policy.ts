...
import { createPrismaAutonomyPolicyRepository } from "@/modules/tenant/infrastructure/prisma-autonomy-policy-repository";
import {
...
export async function updateAutonomyPolicy(input: {
  ...
  return input.prisma.$transaction(async (tx) => {
    const txPolicies = createPrismaAutonomyPolicyRepository(tx);
    ...
    const next = await txPolicies.upsert(input.tenantId, {
      ...parsed,
      disabledAutomations:
        parsed.disabledAutomations ?? previous.disabledAutomations,
    });
